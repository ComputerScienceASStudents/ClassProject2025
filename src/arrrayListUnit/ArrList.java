...
/**
 *
 * The word generic is defined as a characteristic of or relating to a class or
 * group of things that is not specific
 * 
 * In Java, generic classes and methods were introduced to enable
 * programmers to specify (with a single declaration), a set of related methods
 * or a set of related types, respectively
 * 
 * When creating a generic method, the argument and/or return type can be
 * generic and the compiler adapts to the data type sent to it appropriately
 * to do this: use <t> 
 * with a single letter in-between to
 * represent the type which will be later defined, and this is place before the
...
public class ArrList {
    ...
    /**
     * Now, in this unit we will expand our knowledge of how data can be stored
        beyond where we started
        Originally we stored data in variables, then we learned about arrays
        The problem with arrays is that they are a fixed size – so if we want to add
        to a fixed array, we have to remake the array, or limit the number of items
        it stores (like the limit of Student objects we had in our Teacher class
        in the last unit)
        * 
        * If we make the array quite large to accommodate this, then we are
        * “wasting” memory of unused locations
     */
    public ArrList() {
        /*
        The term “abstract data type” or ADT is used to describe programming
        constructs that implement the concept of abstraction, often along with the
        concept of dynamics – so sometimes they are called “abstract dynamic
        types”
        
        Some of the most common types created in this category include: list,
        stack, queue, set (including the table), and the graph
        */
        
        /*
        A linked list is a collection of data objects “linked together” and
        where each object maintains a reference (a “pointer”) to the next
        object in the list
        This is different from an array
        where we have one identifier
        referencing all the items
        Here we design the ADT so that in
        the class we have a reference to
        ...
        */
        ...
        //     the ArrayList - the "right" of the index
        ////////////////////////////////////////////////////////////////////////
        ...
        list.add(index, value); //"add before" method
        System.out.println("Method add(T) success: " + success);
        System.out.println("ArrayList: " + list.toString());
        ...
        index = 3;
        System.out.println("meth get(3) " + list.get(index));
        
        index = 0;
        System.out.println("Method get(0) = " + list.get(index));
        
        index = list.size()-1; //because array size doesnt account for indexing
        ...
        System.out.println("" + list.set(index, value));
